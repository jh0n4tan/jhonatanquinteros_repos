...
const tribe_controller = async(req:Request,res:Response)=>{

       const id_tribe = req.params.id      

       if(!id_tribe || id_tribe.trim().length === 0 ) return res.status(404).send({msg:"Se debe enviar el id de la tribu a consultar"});
